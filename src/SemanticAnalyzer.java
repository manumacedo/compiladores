    ...
    private void recInicializaObjeto() {
        ...
        if(!isDeclaredAnywhere(id)) {
        	...
        	if(!classes.containsKey(unit.getType())) {
        		handler.add(this.currentToken().getLine(), "não se pode usar new com variáveis que não são objetos");
        	}
        }
                ...
                switch (this.currentToken().getType()) {
                    case identificador:
                        ...
                        if(!type.equals(recComplementoAritmetico())) {
                        	handler.add(this.currentToken().getLine(), "Tipos inválidos na expressáo - " + type);
                        }
                        recOpIdLogico();
                        break;
                    case decimal:
